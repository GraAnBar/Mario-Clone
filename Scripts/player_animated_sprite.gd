...
func trigger_animation(velocity: Vector2, direction: int, player_mode: Player.PlayerMode):
	var animatio_prefix = Player.PlayerMode.keys()[player_mode].to_snake_case()
	
	if not get_parent().is_on_floor():
		play("%s_jump" % animatio_prefix)
	
	#handleslide animations
	elif sign(velocity.x) != sign(direction) && velocity.x != 0 && direction != 0:
		play("%s_slide" % animatio_prefix)
		scale.x = direction
	else:
	#sprite direction
		...
		#handle run and idle
		if velocity.x != 0:
			play("%s_run" % animatio_prefix)
		else:
			play("%s_idle" % animatio_prefix)
